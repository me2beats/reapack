-- @description Trim sel items right edges to nearest grid divisions (increase items length)
-- @version 1.0
-- @author me2beats
-- @changelog
--  + init
...
function hello()
  ...
  end_grid = reaper.BR_GetNextGridDivision(end_pos)
  reaper.ApplyNudge(0, 1, 3, 1, end_grid, false, 0)
end

items = reaper.CountSelectedMediaItems(0)
if items > 0 then
  script_title = 'Trim sel items right edges to nearest grid divisions (increase items length)'
