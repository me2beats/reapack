-- @description Set items length to 0.5 (stretch items)
-- @version 1.0
-- @author me2beats
-- @changelog
--  + init
...
if items > 0 then
  ...
  for i = 1, #tb do
    ...
    r.GetSet_LoopTimeRange(1, 0, ts_start, 0.5*(ts_start+ts_end), 0)
    r.Main_OnCommand(41206,0) -- move and stretch item to fit time selection
  end
  for i = 1, #tb do r.SetMediaItemSelected(tb[i], 1) end
  RestoreLoopTimesel()
  r.PreventUIRefresh(-1); r.Undo_EndBlock('set items length to 0.5 (stretch)', -1)
