...
if items > 0 then
  ...
  if retval == true then
    ...
    for i = 0, items-1 do
      ...
      if len and len >= 0 then r.SetMediaItemInfo_Value(item, 'D_FADEINLEN', len) end
      if curve and curve >= -1 and curve <= 1 then r.SetMediaItemInfo_Value(item, 'D_FADEINDIR', curve) end
      if auto then r.SetMediaItemInfo_Value(item, 'D_FADEINLEN_AUTO', auto) end
      if shape then r.SetMediaItemInfo_Value(item, 'C_FADEINSHAPE', shape) end
    end
    r.UpdateArrange()
    r.Undo_EndBlock('Set items fade in', -1)
  else bla() end
else bla() end
