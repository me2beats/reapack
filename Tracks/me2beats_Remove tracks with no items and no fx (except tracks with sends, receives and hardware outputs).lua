-- @description Remove tracks with no items and no fx (except tracks with sends, receives and hardware outputs)
-- @version 1.0
-- @author me2beats
-- @changelog
--  + init
...
if tracks ~= 0 then
    ...
    for cur_tr_number = 1, #t do
      ...
      if t[cur_tr_number][2] == '' then -- is this track empty?
        if reaper.GetTrackNumSends(tr, -1) == 0 then -- if track hasn't sends
          if reaper.GetTrackNumSends(tr, 0) == 0 then -- if track hasn't receives
            if reaper.GetTrackNumSends(tr, 1) == 0 then -- if track hasn't hardware outputs
              if reaper.TrackFX_GetCount(tr) == 0 then -- if track hasn't FX
  ...
  reaper.PreventUIRefresh(-111); reaper.Undo_EndBlock('remove tracks with no items and no fx (except tracks with sends, receives and hardware outputs)', 1)
