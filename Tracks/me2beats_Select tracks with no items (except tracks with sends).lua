-- @description Select tracks with no items (except tracks with sends)
-- @version 1.0
-- @author me2beats
-- @changelog
--  + init
...
if tracks ~= 0 then
  ...
  reaper.PreventUIRefresh(-111); reaper.Undo_EndBlock('select tracks with no items (except tracks with sends', 1)
