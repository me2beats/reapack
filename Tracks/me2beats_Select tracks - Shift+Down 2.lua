-- @description Select tracks - Shift+Down 2
-- @version 1.0
-- @author me2beats
-- @changelog
--  + init
...
if first_sel == last_sel then
  r.Main_OnCommand(40287,0) -- Track: Go to next track (leaving other tracks selected)
else
  if first_sel == last_touched then
    r.SetTrackSelected(first_sel,0)
    local first_sel_num = r.GetMediaTrackInfo_Value(first_sel, 'IP_TRACKNUMBER')
    set_last_touched(r.GetTrack(0,first_sel_num))
  elseif last_sel == last_touched then
    r.Main_OnCommand(40287,0) -- Track: Go to next track (leaving other tracks selected)
  end
end
